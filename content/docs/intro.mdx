...
- **gh CLI isolation** (canary) — every terminal's `gh` commands resolve the right account for their repo, so multiple terminals never fight over gh's one shared global account
